...
LOG1=${WORKDIR}/javis_test.log
LOG2=${WORKDIR}/javis_train.log
LOG3=${WORKDIR}/javis_cancel.log
...
function do_start()
{
	source activate ohif
        cd ${WORKDIR}
        nohup python ${DAEMON1} >> ${LOG1} & nohup python ${DAEMON2} >> ${LOG2} & nohup python ${DAEMON3} >> ${LOG3} & 
}
