...
"""
Model Module, Made by KBS, BJH. JYJ OBS Korea
"""


import tensorflow as tf
import numpy as np
from Layer import (conv3d, activate, pool, deconv3d)


class Unet():
    def __init__(self, act_func, layer_n, n_i_channel=1, n_class=2, cost="mfc", cost_kwargs={}, **kwargs):
        ...
        self.x = tf.placeholder(tf.float32, shape=[1, 128, 256, 256, n_i_channel])  # [None, None, None, None, n_i_channel]
        self.y = tf.placeholder(tf.float32, shape=[1, 128, 256, 256, n_class])
        self.training = tf.placeholder(tf.bool)
        self.keep_prob = tf.placeholder(tf.float32)

        logits = self._construct_unet(m_b_l_number=self.layer_n, **kwargs)
    ...
    def _get_cost(self, logits, cost_name, cost_kwargs):
        ...
        # if regularizer == "L2":
        #     loss += (sum([tf.nn.l2_loss(variable) for variable in self.variables]) ** 2)
        # elif regularizer == "L1":
        #     loss += (sum([tf.nn.l1_loss(variable) for variable in self.variables])**2)

        return loss

    def _construct_unet(self, n_root_channel=2, b_l_number=2, m_b_l_number=8, kernel_size=3, pool_size=2, stride_size=1, **kwargs):
        ...
        depth = kwargs.get("n_layer", 24 - m_b_l_number - 1) // 4
        for d_number in range(depth):
            ...
            for l_number in range(b_l_number):
                ...
                with tf.variable_scope(downname):
                    ...
                    conv = activate(conv, self.training, self.keep_prob, conv_name, act_func=self.act_func)
        ...
        for l_number in range(m_b_l_number):
            conv_name = "middle_conv_" + str(l_number + 1)
        ...
        for d_number in range(depth - 1, -1, -1):
            ...
            for l_number in range(b_l_number):
                ...
                with tf.variable_scope(upname):
                    conv_name = "conv" + str(l_number + 1)
                    conv = conv3d(conv, n_channel, kernel_size, stride_size, dilation_rate, conv_name)
                    activate(conv, self.training, self.keep_prob, conv_name, act_func=self.act_func)
                    # conv=tf.concat([conv, conv_dict[]], axis=-1)
